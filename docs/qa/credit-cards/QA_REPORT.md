...
- Badge owns normal/glass context. Because the installed React components render supplied Icon slots verbatim, the same Badge mode objects are forwarded to those Icons so the Figma cascade resolves through Coin tokens without literal color overrides.
